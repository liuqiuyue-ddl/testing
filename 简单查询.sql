...
6、排序 order by 
		...
		经验：
			读：当我打开一个库，我们可以用逆向工程来看清楚表关系
					耐心一点，读读这些表的信息，一般这些信息都会很清晰的用英文处理
					我一般不会傻傻的去读所有字段并尝试背下来的傻子做法
						我只做一个事，看字段中的数据是什么类型！
							如果是数字，我就数字过滤；
										字符串		字符串
										空				空
									有相同值，  立刻想到可以分组且分组有意义
						做完这个事，我开始考虑这个表和其他表有没有关系
							如果有关系，我就会想到用子查询来关联处理他们之间的关系
			写：
...
select ename,deptno,(sal+ifnull(comm,0))*12 year_sal from emp where empno=7839;
